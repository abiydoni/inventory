...
if (isset($_GET['hapus']) && Helper::validateCSRF($_GET['token'] ?? '')) {
    try {
        ...
        echo "<script>Swal.fire('Sukses', 'Supplier berhasil dihapus', 'success').then(() => {location.href='?page=supplier'});</script>";
    } catch (Exception $e) {
        echo "<script>Swal.fire('Error', 'Gagal menghapus supplier: " . addslashes($e->getMessage()) . "', 'error');</script>";
    }
}
...
$totalRows = $db->fetch("SELECT COUNT(*) as total FROM supplier $whereClause", $params)['total'];
$totalPages = ceil($totalRows / $perPage);

$rows = $db->fetchAll("SELECT * FROM supplier $whereClause ORDER BY id DESC LIMIT ? OFFSET ?", 
    ...
    <div class="px-6 py-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Kelola data supplier untuk transaksi pembelian</p>
            </div>
            <button onclick="openTambah()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
...
<div class="bg-white border-b border-gray-200 px-6 py-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        ...
        <div class="flex items-center space-x-4">
            <div class="text-center">
                <div class="text-2xl font-bold text-green-600"><?php echo $totalRows; ?></div>
    ...
    <div class="mt-4">
        ...
        <?php if (!empty($search)): ?>
            <a href="?page=supplier" class="ml-2 bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg transition-colors">
...
<div class="bg-white shadow-sm rounded-lg overflow-hidden">
    ...
    <?php if (empty($rows)): ?>
        <div class="text-center py-12">
            <i class='bx bx-store text-6xl text-gray-300 mb-4'></i>
            <h3 class="text-lg font-medium text-gray-900 mb-2">Belum ada supplier</h3>
            <p class="text-gray-500 mb-4">Mulai dengan menambahkan supplier pertama Anda</p>
            <button onclick="openTambah()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg">
                <i class='bx bx-plus mr-2'></i>Tambah Supplier
            </button>
        </div>
    <?php else: ?>
    ...
    <div class="bg-white px-6 py-4 border-t border-gray-200">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-700">
                Menampilkan <?php echo ($offset + 1); ?> - <?php echo min($offset + $perPage, $totalRows); ?> dari <?php echo $totalRows; ?> supplier
            </div>
            <div class="flex space-x-2">
                <?php if ($page > 1): ?>
                    <a href="?page=supplier&p=<?php echo ($page - 1); ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>" 
                ...
                <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                    <a href="?page=supplier&p=<?php echo $i; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>" 
                       class="px-3 py-2 border border-gray-300 rounded-lg text-sm <?php echo $i === $page ? 'bg-green-50 text-green-600 border-green-600' : 'text-gray-700 hover:bg-gray-50'; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>
...
function performSearch() {
    ...
    location.href = `?page=supplier&search=${encodeURIComponent(search)}`;
}
...
function openTambah() {
    ...
    function generateKode() {
        ...
        return 'SUPP' + (timestamp + random).slice(-4);
    }
    ...
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            return fetch('views/supplier/api.php', { method: 'POST', body: formData })
                .then(r => r.json())
...
function edit(id) {
    fetch(`views/supplier/api.php?action=get&id=${id}&token=<?php echo $csrf_token; ?>`)
        .then(r => r.json())
        .then(d => {
            if (!d || d.status === 'error') {
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            location.href = `?page=supplier&hapus=${id}&token=${token}`;
        }
    });
}
</script>
