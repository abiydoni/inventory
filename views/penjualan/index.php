...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['aksi'])) {
    ...
    if ($aksi=='simpan') {
        ...
        // simpan penjualan
        $s = $db->prepare('INSERT INTO penjualan (tanggal,pelanggan,subtotal,diskon,pajak,total) VALUES(:t,:pel,:sub,:disc,:pajak,:tot)');
        $s->execute([':t'=>$tanggal,':pel'=>$pelanggan,':sub'=>$subtotal,':disc'=>$diskon,':pajak'=>$pajak,':tot'=>$total]);
        $pid = $db->lastInsertId();
        ...
        // jurnal - debit kas, kredit penjualan
        ...
        $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,0,:k,:ket)')
           ->execute([':t'=>$tanggal,':akun'=>'Pendapatan',':k'=>$total,':ket'=>'Penjualan #'.$pid]);
...
<div class="bg-white p-4 shadow rounded">
  <div class="mb-3">
    ...
    <input id="pelanggan" class="border p-2 w-full" placeholder="Nama pelanggan">
  </div>
  <div class="mb-3">
  ...
  <div>
    ...
    <div class="flex gap-2">
      <select id="pilih_barang" class="border p-2 flex-1">
        ...
        <?php foreach($barang as $b) echo "<option value='{$b['id']}' data-harga='{$b['harga']}'>{$b['kode']} - {$b['nama']} (Stok: {$b['stok']})</option>"; ?>
      </select>
  ...
  <div class="mt-4">
    <button onclick="simpanPenjualan()" class="bg-green-500 text-white px-4 py-2 rounded">Simpan Penjualan</button>
  </div>
</div>
<script>
let items = [];
function tambahItem(){
  ...
  const stokText = sel.options[sel.selectedIndex].text;
  const harga = Number(sel.options[sel.selectedIndex].dataset.harga || 0);
  const qty = Number(document.getElementById('qty').value || 0);
  items.push({id:id, nama:stokText, qty:qty, harga:harga});
  renderItems();
}
...
function simpanPenjualan(){
  ...
  data.append('pelanggan', document.getElementById('pelanggan').value || '');
  ...
  fetch('', {method:'POST', body:data}).then(r=>r.json()).then(j=>{
    if(j.status=='ok') Swal.fire('Sukses','Penjualan tersimpan','success').then(()=>location.reload()); else Swal.fire('Error','Gagal','error');
  });
}
</script>
