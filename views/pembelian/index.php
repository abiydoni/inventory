...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['aksi'])) {
    ...
    if ($aksi=='simpan') {
        ...
        $jenis_pembayaran = $_POST['jenis_pembayaran'] ?? 'tunai';
        $status_pembayaran = $_POST['status_pembayaran'] ?? 'lunas';
        ...
        // simpan pembelian
        $s = $db->prepare('INSERT INTO pembelian (tanggal,supplier_id,jenis_pembayaran,status_pembayaran,subtotal,diskon,pajak,total) VALUES(:t,:sup,:jp,:sp,:sub,:disc,:pajak,:tot)');
        $s->execute([':t'=>$tanggal,':sup'=>$supplier_id,':jp'=>$jenis_pembayaran,':sp'=>$status_pembayaran,':sub'=>$subtotal,':disc'=>$diskon,':pajak'=>$pajak,':tot'=>$total]);
        $pid = $db->lastInsertId();
        ...
        if ($jenis_pembayaran == 'cash') {
            ...
            $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,:d,0,:ket)')
               ->execute([':t'=>$tanggal,':akun'=>'Persediaan Barang Dagang',':d'=>$total,':ket'=>'Pembelian #'.$pid]);
            ...
            $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,:d,0,:ket)')
               ->execute([':t'=>$tanggal,':akun'=>'Persediaan Barang Dagang',':d'=>$total,':ket'=>'Pembelian #'.$pid]);
            $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,0,:k,:ket)')
               ->execute([':t'=>$tanggal,':akun'=>'Bank',':k'=>$total,':ket'=>'Pembelian #'.$pid]);
        } else {
            // Debit Persediaan, Kredit Hutang Usaha
            $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,:d,0,:ket)')
               ->execute([':t'=>$tanggal,':akun'=>'Persediaan Barang Dagang',':d'=>$total,':ket'=>'Pembelian #'.$pid]);
            $db->prepare('INSERT INTO jurnal (tanggal,akun,debit,kredit,keterangan) VALUES(:t,:akun,0,:k,:ket)')
               ->execute([':t'=>$tanggal,':akun'=>'Hutang Usaha',':k'=>$total,':ket'=>'Pembelian #'.$pid]);
        }
...
$suppliers = $db->query('SELECT id,kode,nama,alamat FROM supplier ORDER BY nama')->fetchAll(PDO::FETCH_ASSOC);
?>
...
<div class="bg-white p-4 shadow rounded">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
    <div>
      ...
      <div x-data="supplierDropdown()" class="relative">
        <input 
          x-ref="input"
          @input="search = $event.target.value; showDropdown = true"
          @click="showDropdown = true"
          @click.away="showDropdown = false"
          :value="selectedSupplier ? selectedSupplier.nama : search"
        ...
        <div x-show="showDropdown" x-transition class="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          <template x-for="supplier in filteredSuppliers" :key="supplier.id">
            ...
            >
              ...
              <div class="text-sm text-gray-600" x-text="supplier.alamat"></div>
            </div>
          </template>
          <div x-show="filteredSuppliers.length === 0" class="px-3 py-2 text-gray-500">
            Tidak ada supplier ditemukan
          </div>
        </div>
        <input type="hidden" id="supplier_id" x-model="selectedSupplierId">
      </div>
    </div>
    <div>
  ...
  <div class="text-center">
    <button onclick="simpanPembelian()" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg text-lg font-medium">
      <i class='bx bx-save mr-2'></i>Simpan Pembelian
    </button>
  </div>
</div>
...
function simpanPembelian(){
  ...
  if(!supplierId) return Swal.fire('Pilih supplier dulu');
  ...
  data.append('status_pembayaran', document.getElementById('jenis_pembayaran').value); // Assuming status is same as payment type for now
  ...
  fetch('', {method:'POST', body:data}).then(r=>r.json()).then(j=>{
    if(j.status=='ok') {
      Swal.fire('Sukses','Pembelian tersimpan','success').then(()=>location.reload());
    } else {
